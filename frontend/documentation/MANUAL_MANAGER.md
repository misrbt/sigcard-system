...
flowchart TD
    ...
    F --> G([BRANCH DASHBOARD])

    G --> H{What do you need?}

    H -- Enroll customer --> I[Upload Sigcard wizard]
    I --> I1[Fill info → Upload Sigcard → NAIS → Data Privacy → Other → Submit]
    I1 --> I2([Customer enrolled])

    H -- Manage customer --> J[Customer Profiles → View]
    J --> K{Action?}
    K -- Change status --> K1[Change Status → Choose → Date → Docs → Save]
    K -- Replace doc --> K2[Replace icon → Upload → Upload Files]
    K -- Add account --> K3[Add Account → Fill → Upload → Submit]

    H -- Browse docs --> L[Documents page]
    L --> L1[Filter / Search / View / Download]

    H -- Branch analytics --> M[Status Tracking]
    M --> M1[Charts · Monthly breakdown · Trends]

    H -- Profile --> N[Click your name → Profile]
    N --> N1[Change Password / Manage 2FA]

    style A fill:#1e3a5f,color:#fff
    style G fill:#065f46,color:#fff
    style I2 fill:#065f46,color:#fff
```
