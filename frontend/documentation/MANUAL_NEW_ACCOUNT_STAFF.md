...
```mermaid
flowchart TD
    A([START]) --> B[Admin gives you email address]
    B --> C[Open DIGICUR login page]
    C --> D[Enter email + abc_123 → Sign In]
    D --> E{2FA required?}
    E -- Yes --> F[Scan QR code\nEnter 6-digit code\nComplete Setup]
    F --> G
    ...
    J -- Enroll new customer --> K[Click Upload Sigcard]
    K --> K1[Step 1: Customer info\nname · account no. · risk · status · branch]
    K1 --> K2[Step 2: Signature Card\nfront AND back — required]
    K2 --> K3[Step 3: NAIS Form\noptional — can skip]
    K3 --> K4[Step 4: Data Privacy\nfront AND back — required]
    K4 --> K5[Step 5: Other Docs\noptional]
    K5 --> K6[Click Submit]
    K6 --> K7([Customer saved!])

    J -- Find customer --> L[Click Customer Profiles]
    L --> L1[Search by name or account number]
    L1 --> L2[Click View]
    L2 --> L3([Customer Profile opens])
    L3 --> L4{What action?}

    L4 -- Change status --> M1[Click Change Status]
    M1 --> M2[Choose status · Set date · Upload docs]
    M2 --> M3([Status saved to history])

    L4 -- Replace document --> N1[Click Replace on image]
    N1 --> N2[Upload new file → Upload Files]
    N2 --> N3([Old file archived · New file saved])

    L4 -- Add account --> O1[Click Add Account]
    O1 --> O2[Fill details · Upload docs · Submit]
    O2 --> O3([New account tab added])

    J -- Check analytics --> P[Click Status Tracking\nSee charts and monthly breakdown]

    J -- Manage profile --> Q[Click your name]
    Q --> Q1[Change Password / Manage 2FA]

    style A fill:#1e3a5f,color:#fff
    style I fill:#065f46,color:#fff
    style K7 fill:#065f46,color:#fff
    style M3 fill:#065f46,color:#fff
    style N3 fill:#065f46,color:#fff
    style O3 fill:#065f46,color:#fff
```
